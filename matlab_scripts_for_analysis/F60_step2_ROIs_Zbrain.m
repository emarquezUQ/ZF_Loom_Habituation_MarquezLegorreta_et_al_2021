...
load('final_F60_step1_2.mat','MatFiles','ZS_f60','idx_Fish_f60','idx_Plane_f60','rawregressF60','ZS_short_F60','gooodmaps');
load('final_F60_step1_3_short_correction.mat');


MatFiles_f60=MatFiles;

clearvars MatFiles

Fish_list_f60=num2str(unique(idx_Fish_f60));



%%%% this is for only f60 fish

%%%% now, this is the part where i used the warped datasets to get the
%%%% brain locations of my ROIs.
list=str2num(Fish_list_f60);
...
for i=1:length(CSV_Files)
    ...
    for j=1:length(Fish_list_f60)
     
    if str2num(Fishname)==list(j)
    
    %if ismember(str2num(Fishname),str2num(Fish_list_f60))
    
    temp_warp=csvread(CSV_Files(i).name,1);    
    ROIs(j).name=Fishname;    
    ROIs(j).coord=temp_warp(:,1:3);
    ROIs(j).idx=temp_warp(:,5);
    truth(j)=size(temp_warp,1)==sum(idx_Fish_f60==str2num(Fishname)) %%% important!! i need to have an idx_Fish with all the fish.
...
MatFiles_names={MatFiles_f60.name};
for fish_nb=1:length(Fish_list_f60)
    
    if ismember(str2num(Fishname),str2num(Fish_list_f60))
    
    temp_warp=str2num(Fish_list_f60);
    temp_warp=temp_warp(fish_nb);
	IndexC=strfind({MatFiles_f60.name}, strcat('_fish',num2str(temp_warp),'_')); 
    ...
    for file_nb=1:length(MatFiles_fish)
        if MatFiles_fish(file_nb)==1
            numbersForROIs=[1 MatFiles_f60(1).GoodNumber];
            
        else
            numbersForROIs=[MatFiles_f60(MatFiles_fish(file_nb)-1).GoodNumber+1 MatFiles_f60(MatFiles_fish(file_nb)).GoodNumber]; 
        end
    ...
    if ~length(Sort_ROIs)-temp_nb==sum(idx_Fish_f60==temp_warp)
...
ROI_fish(Sort_ROIs,:)=ROI_pool; %%% Sort_ROIs and ROI_pool need, to have the same amount of elements .doesnt work... different numbers. ask gilles. I think i know, it is cause i just have the warped images of f60 and s60 but not from the rest. i changed that but still doesnt work...
...
%figure;
hold on; scatter(ROI_temp2(find(idx_Fish_f60==4),1),ROI_temp2(find(idx_Fish_f60==4),2)); %%% this is my rotation
%%%as I have it at the moment is looking to the right. 

%%% chekcing in 3D
figure;scatter3(ROI_temp2(find(idx_Fish_f60==1),1),ROI_temp2(find(idx_Fish_f60==1),2),ROI_temp2(find(idx_Fish_f60==1),3)); %%% this is my rotation
...
save('BrainReg_F60.mat','PerBrainRegions','ROI_temp2','RegionList2','-v7.3');

%%

%%% to have a panoramic view of the CL4
figure;scatter(ROI_temp2(idx_rsq_test_f60short3,1),ROI_temp2(idx_rsq_test_f60short3,2));
hold on;
for i=gooodmaps

idx_temp1=find(High_corr_Nb_f60_short==i);  %%% 1 is fasthab in s20

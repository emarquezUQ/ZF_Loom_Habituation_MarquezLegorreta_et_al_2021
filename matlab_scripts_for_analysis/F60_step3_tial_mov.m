...
%%% it with a linear regression using rsq=0.1 as threshold.
...
load('final_F60_step1_2.mat','ZS_f60','idx_Fish_f60','idx_Plane_f60','rawregressF60');
load('final_F60_step1_3_short_correction.mat');
load('Zbrain_Masks.mat');

load allfish_f60looms_tailmov



%%

allfish_f60looms_tailmov=cell2mat(allfish_f60looms_tailmov(2:end,:));


%%%first to generate the regressors for the movements. 
Fish_movements={}; %%%first we make a structure to put the data in
for fish=1:size(allfish_f60looms_tailmov,2) %%%we make a variable 'fish' as long as the columns from the Behavioural_responses variable which has the data copied from excel
    for i=1:2 %%%% Note: here I had a loop to work with the different strenght responses but we decided to only take the strong responses (1 and 2).
        %%% I have resposnes from 1-4 depending on the strenght of the moment.
        
        temp=find(allfish_f60looms_tailmov(:,fish)==i); %%%to find the type of responses in turn. 
        ...
        temp(temp<1)=[];
        temp(temp>(length(allfish_f60looms_tailmov)-1))=[];        
    ...
    if fish==11 %%at the moment is 11 (for f60)
        temp=find(allfish_f60looms_tailmov(:,fish)==2);
        temp=round(temp/5);
        temp(temp<1)=[];
        temp(temp>(length(allfish_f60looms_tailmov)-1))=[];        
...
Movements=zeros(size(allfish_f60looms_tailmov,2)-1,1,size(ZS_f60,2)); %%%to mae a 3D array the size of the number of fish (18 at the moment), the type of movement (1) and the lenght of the SPIM movies (depending on the variable). 
...
for fish=1:size(allfish_f60looms_tailmov,2)-1
...
Movements=Movements(:,:,1:size(ZS_f60,2));
...
Fish_with_behav_f60=intersect(unique(idx_Fish_f60),Fish_with_behav_f60);

Fish_with_behav_f60=Fish_with_behav_f60'

%%% to show where when all the looms are presented


loomtimes=zeros(1,size(ZS_f60,2)); %%%to make a 3D array the size of the number of fish, the type of movement (1) and the lenght of the SPIM movies (depending on the variable). 

idx=Fish_movements{11,1};%%%%to take the timepoints of the movements.  
...
figure;plot(loomtimes(1,ZS_short_F60));
...
%%%%first 3 looms. for f60 it would be from 180 onwards. This is because
...
counter=1;
rsquare_loom_allmov={};
for fish= Fish_with_behav_f60
   if ismember(fish,unique(idx_Fish_f60))  
    temp_ZS=ZS_f60(find(idx_Fish_f60==fish),:);ModelResults_temp=[];
    %temp_ZS=ZS_f60(find(idx_Fish_f60==fish),:);ModelResults_temp=[];
        ...
        mdl=fitlm(squeeze(squeeze(Movements(find(Fish_with_behav_f60==fish),:)))',temp_ZS(i,:));
...
for fish= Fish_with_behav_f60
   if ismember(fish,unique(idx_Fish_f60))  
    temp_idx=find(idx_Fish_f60==fish); idx_Mov_temp=[];   
...
figure; imagesc(ZS_f60(idx_rsq_Mov,ZS_short_F60), [-0.5 4]);colormap hot
...
save('Tail_mov_F60.mat','idx_rsq_Mov','rsquare_loom_allmov2','ModelResults_allmovPerFish','loomtimes','Fish_with_behav_f60','Movements','-v7.3');
...
%%%% It makes a figure with a subplot of each fish. 
    ...
    counter=1;
    figure; 
    xplot=floor(sqrt(length(Fish_with_behav_f60)));yplot=ceil(length(Fish_with_behav_f60)/xplot);
    for fish=Fish_with_behav_f60
        if ismember(fish,unique(idx_Fish_f60)) %%isempty(Correlation_movement_post3{fish})~=1 % idx_Fish_f60==Fish_with_behav_f60(fish)
        temp_ZS=ZS_f60(find(idx_Fish_f60==fish),:);        
        rsq_temp=[ModelResults_allmovPerFish{1,counter}.rsquared];
...
%       Movements2=squeeze(Movements2);
        plot(Movements2(find(Fish_with_behav_f60==fish),:))
        plot(loomtimes);
        hold off
